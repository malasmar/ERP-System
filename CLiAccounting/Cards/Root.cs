...
namespace CLiAccounting.Cards
{
    public class Root
    {
        ...
        public List<Root> GetList(string DB)
        {
            List<Root> items = new List<Root>();
            string selQuery = "select top 100 percent * from accCard_Accounts where [acc_Level]=1 order by [acc_Code] ";
        ...
        public Root GetItem(string DB,Guid? Key)
        {
            Root item = new Root();
            if (Key == null)
                return item;
        ...
        public static void Insert(string DB, Root item)
        {
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                comm.Parameters.Add("@acc_Parent", SqlDbType.NVarChar, 15).Value =  "";
                comm.Parameters.Add("@acc_BalanceItem", SqlDbType.UniqueIdentifier).Value = iCore.IsNullRtDbNull(item.BalanceItem);
                comm.Parameters.Add("@acc_Kind", SqlDbType.Int).Value = item.Kind;
                comm.Parameters.Add("@acc_Level", SqlDbType.Int).Value = 1;
                comm.Parameters.Add("@acc_Financial", SqlDbType.Bit).Value = false;
                comm.Parameters.Add("@acc_Source", SqlDbType.Bit).Value = false;
                comm.Parameters.Add("@acc_Category", SqlDbType.Int).Value = 0;
                ...
                comm.Parameters.Add("@acc_CloseSubAccounts", SqlDbType.Bit).Value = false;
                con.Open();
                comm.ExecuteNonQuery();
            }
        }

        public static void Update(string DB, Root item)
        {
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                System.Text.StringBuilder str = new System.Text.StringBuilder();
                ...
                comm.Parameters.Add("@acc_Category", SqlDbType.Int).Value = 0;
                comm.Parameters.Add("@acc_Disable", SqlDbType.Bit).Value = item.Disable;
                comm.Parameters.Add("@acc_IsParent", SqlDbType.Bit).Value = true;
                comm.Parameters.Add("@acc_DC", SqlDbType.Bit).Value = item.DC;
                con.Open();
                comm.ExecuteNonQuery();
            }
        ...
        public static int Delete(string DB, Guid? Key)
        {
            ...
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                string delQuery = "EXEC spfinDelete_Account @Key ";
                SqlCommand comm = new SqlCommand();
                comm.Connection = con;
                comm.CommandType = CommandType.Text;
