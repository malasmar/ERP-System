...
namespace CLiMobile.SelfService.MyRequests
{
    public class MyLoan
    {
        ...
        public List<MyLoan> GetList(string DB,Guid? Key)
        {
            List<MyLoan> items = new List<MyLoan>();
            if (Key == null)
                return items;
            string selQuery = "select top 100 percent * from dbo.fnAppSelfService_MyRequest_Loan(@Key) order by [Create] desc ";
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                while (reader.Read())
                {
                    ...
                    item.MonthlyAmount = Convert.ToDecimal(reader["MonthlyAmount"]);
