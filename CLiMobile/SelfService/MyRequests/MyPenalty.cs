...
namespace CLiMobile.SelfService.MyRequests
{
    public class MyPenalty
    {
        ...
        public List<MyPenalty> GetList(string DB,Guid? Key)
        {
            List<MyPenalty> items = new List<MyPenalty>();
            if (Key == null)
                return items;
            string selQuery = "select top 100 percent * from dbo.fnAppSelfService_MyRequest_Penalty(@Key) order by [Create] desc ";
