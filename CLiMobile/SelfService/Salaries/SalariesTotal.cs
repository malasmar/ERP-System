...
namespace CLiMobile.SelfService.Salaries
{
    public class SalariesTotal
    {
        ...
        public SalariesTotal GetItem(string DB,Guid? Key)
        {
            SalariesTotal item = new SalariesTotal();
            if (Key == null)
                return item;

            string selQuery = "select top 100 percent * from dbo.fnAppSelfService_SalariesTotal(@Key) ";
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                while (reader.Read())
                {
                    item.Months = Convert.ToInt32(reader["Months"]);
