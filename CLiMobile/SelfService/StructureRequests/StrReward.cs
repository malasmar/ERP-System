...
namespace CLiMobile.SelfService.StructureRequests
{
    public class StrReward
    {
        ...
        public List<StrReward> GetList(string DB, Guid? Key)
        {
            List<StrReward> items = new List<StrReward>();
            if (Key == null)
                return items;

            string selQuery = "select top 100 percent * from dbo.fnAppSelfService_SturctureReward(@Key) order by [Create] ";
