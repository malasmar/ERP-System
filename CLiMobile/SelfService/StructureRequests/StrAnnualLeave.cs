...
namespace CLiMobile.SelfService.StructureRequests
{
    public class StrAnnualLeave
    {
        ...
        public List<StrAnnualLeave> GetList(string DB,Guid? Key)
        {
            List<StrAnnualLeave> items = new List<StrAnnualLeave>();
            string selQuery = "select top 100 percent * from dbo.fnAppSelfService_SturctureAnnualLeave(@Key) order by [Create]";
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                while (reader.Read())
                {
                    StrAnnualLeave item = new StrAnnualLeave();
