...
namespace CLiMobile.SelfService.StructureRequests
{
    public class StrAdvance
    {
        ...
        public List<StrAdvance> GetList(string DB,Guid? Key)
        {
            List<StrAdvance> items = new List<StrAdvance>();
            if (Key == null)
                return items;

            string selQuery = "select top 100 percent * from dbo.fnAppSelfService_SturctureAdvance(@Key) order by [Create] ";
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                while (reader.Read())
                {
                    StrAdvance item = new StrAdvance();
