...
namespace CLiMobile.SelfService.RequestDetails
{
    public class DetailsPenalty
    {
        ...
        public DetailsPenalty GetItem(string DB,Guid? Key)
        {
            DetailsPenalty item = new DetailsPenalty();
            if(Key==null)
                return item;
            string selQuery = "select top 100 percent * from dbo.fnAppSelfService_RequestDetailsPenalty(@Key) ";
