...
namespace CLiMobile.SelfService.RequestDetails
{
    public class DetailsLoan
    {
        ...
        public decimal LoanHistory { get; set; }
        public decimal LoanClosed { get; set; }
        public decimal LoanReminig { get; set; }
        public Data.PersonDetails PersonDetails { get; set; }
        public PreviousApproval PreviousApproval { get; set; }
        public DetailsLoan GetItem(string DB, Guid? Key)
        {
            DetailsLoan item = new DetailsLoan();
            if (Key == null)
                return item;

            string selQuery = "select top 100 percent * from hrRequest_Loan where [req_Key]=@Key ";
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                while (reader.Read())
                {
                    ...
                    item.LoanHistory = core.GetEmployeeLoan(DB, iCore.IsDbNullRtNull(reader["req_Employee"]));
                    item.LoanClosed = core.GetEmployeeClosedLoan(DB, iCore.IsDbNullRtNull(reader["req_Employee"]));
                    item.LoanReminig = item.LoanHistory - item.LoanClosed;
                }
                reader.Close();
            }
            return item;
