...
namespace CLiFinancial.Cards.Groups
{
    public class TransactionCategories
    {
        ...
        public List<TransactionCategories> GetList(string DB)
        {
            List<TransactionCategories> items = new List<TransactionCategories>();
            ...
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                while (reader.Read())
                {
                    TransactionCategories item = new TransactionCategories();
        ...
        public TransactionCategories GetItem(string DB,Guid? Key)
        {
            TransactionCategories item = new TransactionCategories();
            item.No = MaxNo(DB);
            if (Key == null)
                return item;
            string selQuery = "select top 100 percent * from finCard_CategoriesTransaction where [cat_Key]=@Key ";
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                con.Open();
                SqlCommand com = new SqlCommand();
        ...
        public static void Insert(string DB, TransactionCategories item)
        {
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                System.Text.StringBuilder str = new System.Text.StringBuilder();
                str.Clear();
                str.Append("INSERT INTO finCard_CategoriesTransaction");
                str.Append("([cat_No]");
        ...
        public static void Update(string DB, TransactionCategories item)
        {
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                System.Text.StringBuilder str = new System.Text.StringBuilder();
                str.Clear();
                str.Append("Update finCard_CategoriesTransaction SET ");
        ...
        public static int Delete(string DB, Guid? Key)
        {
            ...
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                string delQuery = " Delete from [finCard_CategoriesTransaction] where [cat_Key]=@Key";
                SqlCommand comm = new SqlCommand();
                comm.Connection = con;
                comm.CommandType = CommandType.Text;
        ...
        public int MaxNo(string DB)
        {
            ...
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                string selQuery = "select top 100 percent isnull(max([cat_No])+1,1) from [finCard_CategoriesTransaction]";
                con.Open();
                SqlCommand com = new SqlCommand();
                com.CommandText = selQuery;
                com.CommandType = CommandType.Text;
