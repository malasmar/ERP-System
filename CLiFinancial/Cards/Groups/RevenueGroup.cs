...
namespace CLiFinancial.Cards.Groups
{
    public class RevenueGroup
    {
        ...
        public List<RevenueGroup> GetList(string DB)
        {
            List<RevenueGroup> items = new List<RevenueGroup>();
            string selQuery = "select top 100 percent *,(select top 100 percent count(x.[rev_Group]) from dbo.finCard_Revenue x where x.rev_Group = grp_Key) as [Relations] from finCard_Group_Revenue order by [grp_No] ";
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                while (reader.Read())
                {
                    RevenueGroup item = new RevenueGroup();
        ...
        public RevenueGroup GetItem(string DB,Guid? Key)
        {
            RevenueGroup item = new RevenueGroup();
            item.No = MaxNo(DB);
            if (Key == null)
                return item;
            string selQuery = "select top 100 percent * from finCard_Group_Revenue where [grp_Key]=@Key ";
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                con.Open();
                SqlCommand com = new SqlCommand();
        ...
        public static void Insert(string DB, RevenueGroup item)
        {
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                System.Text.StringBuilder str = new System.Text.StringBuilder();
                str.Clear();
                str.Append("INSERT INTO finCard_Group_Revenue");
                str.Append("([grp_No]");
        ...
        public static void Update(string DB, RevenueGroup item)
        {
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                System.Text.StringBuilder str = new System.Text.StringBuilder();
                str.Clear();
                str.Append("Update finCard_Group_Revenue SET ");
        ...
        public static int Delete(string DB, Guid? Key)
        {
            ...
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                string delQuery = " Delete from [finCard_Group_Revenue] where [grp_Key]=@Key";
                SqlCommand comm = new SqlCommand();
                comm.Connection = con;
                comm.CommandType = CommandType.Text;
        ...
        public int MaxNo(string DB)
        {
            ...
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                string selQuery = "select top 100 percent isnull(max([grp_No])+1,1) from [finCard_Group_Revenue]";
                con.Open();
                SqlCommand com = new SqlCommand();
                com.CommandText = selQuery;
                com.CommandType = CommandType.Text;
