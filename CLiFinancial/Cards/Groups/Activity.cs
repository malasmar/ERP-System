...
namespace CLiFinancial.Cards.Groups
{
    public class Activity
    {
        ...
        public List<Activity> GetList(string DB)
        {
            ...
            string selQuery = "select top 100 percent *,(select top 100 percent count(x.[ca_Activity]) from dbo.finCard_CurrentAccount x where x.ca_Activity = grp_Key) as [Relations] from finCard_Group_Activity order by [grp_No] ";
        ...
        public Activity GetItem(string DB,Guid? Key)
        {
            ...
            if (Key == null)
                return item;
            string selQuery = "select top 100 percent * from finCard_Group_Activity where [grp_Key]=@Key ";
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                con.Open();
                SqlCommand com = new SqlCommand();
        ...
        public static void Insert(string DB, Activity item)
        {
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                str.Clear();
                str.Append("INSERT INTO finCard_Group_Activity");
                str.Append("([grp_No]");
        ...
        public static void Update(string DB, Activity item)
        {
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                System.Text.StringBuilder str = new System.Text.StringBuilder();
                str.Clear();
                str.Append("Update finCard_Group_Activity SET ");
        ...
        public static int Delete(string DB, Guid? Key)
        {
            ...
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                string delQuery = " Delete from [finCard_Group_Activity] where [grp_Key]=@Key";
                SqlCommand comm = new SqlCommand();
                comm.Connection = con;
                comm.CommandType = CommandType.Text;
        ...
        public int MaxNo(string DB)
        {
            ...
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                string selQuery = "select top 100 percent isnull(max([grp_No])+1,1) from [finCard_Group_Activity]";
                con.Open();
                SqlCommand com = new SqlCommand();
                com.CommandText = selQuery;
                com.CommandType = CommandType.Text;
