...
{
    public class CurrentAccountGroup
    {
        ...
        public List<CurrentAccountGroup> GetList(string DB)
        {
            List<CurrentAccountGroup> items = new List<CurrentAccountGroup>();
            string selQuery = "select top 100 percent *,(select top 100 percent count(x.[ca_Group]) from dbo.finCard_CurrentAccount x where x.ca_Group = grp_Key) as [Relations] from finCard_Group_CurrentAccount order by [grp_No] ";
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                while (reader.Read())
                {
                    CurrentAccountGroup item = new CurrentAccountGroup();
        ...
        public CurrentAccountGroup GetItem(string DB,Guid? Key)
        {
            CurrentAccountGroup item = new CurrentAccountGroup();
            item.No = MaxNo(DB);
            if (Key == null)
                return item;
            string selQuery = "select top 100 percent * from finCard_Group_CurrentAccount where [grp_Key]=@Key ";
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                con.Open();
                SqlCommand com = new SqlCommand();
        ...
        public static void Insert(string DB, CurrentAccountGroup item)
        {
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                System.Text.StringBuilder str = new System.Text.StringBuilder();
                str.Clear();
                str.Append("INSERT INTO finCard_Group_CurrentAccount");
                str.Append("([grp_No]");
        ...
        public static void Update(string DB, CurrentAccountGroup item)
        {
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                System.Text.StringBuilder str = new System.Text.StringBuilder();
                str.Clear();
                str.Append("Update finCard_Group_CurrentAccount SET ");
        ...
        public static int Delete(string DB, Guid? Key)
        {
            ...
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                string delQuery = " Delete from [finCard_Group_CurrentAccount] where [grp_Key]=@Key";
                SqlCommand comm = new SqlCommand();
                comm.Connection = con;
                comm.CommandType = CommandType.Text;
        ...
        public int MaxNo(string DB)
        {
            ...
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                string selQuery = "select top 100 percent isnull(max([grp_No])+1,1) from [finCard_Group_CurrentAccount]";
                con.Open();
                SqlCommand com = new SqlCommand();
                com.CommandText = selQuery;
                com.CommandType = CommandType.Text;
