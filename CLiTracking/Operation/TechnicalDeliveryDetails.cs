...
{
    public class TechnicalDeliveryDetails
    {
        ...
        public List<TechnicalDeliveryDetails> GetList(string DB,Guid? Key)
        {
            List<TechnicalDeliveryDetails> items = new List<TechnicalDeliveryDetails>();
            if (Key == null)
                return items;
            ...
            string selQuery =
                ...
                " FROM [dbo].[TrackingOperation_TechnicalDeliveryDetails] d " +
                " LEFT JOIN invCard_StockItem AS s " +
                " ON d.[dev_Item] = s.item_Key " +
                 " where dev_OperationKey=@Key order by s.item_Code,[dev_SerialNo] ";
            ...
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                while (reader.Read())
                {
                    TechnicalDeliveryDetails item = new TechnicalDeliveryDetails();
