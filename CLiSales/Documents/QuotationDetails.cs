...
namespace CLiSales.Documents
{
    public class QuotationDetails
    {
        ...
        public List<QuotationDetails> GetList(string DB,string xLan,Guid? Key)
        {
            List<QuotationDetails> items = new List<QuotationDetails>();
            if (Key == null)
                return items;

            string selQuery = "select top 100 percent * from SalesDocument_QuotationDetails where [sal_OperationKey]=@key order by [sal_Index] ";
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                while (reader.Read())
                {
                    QuotationDetails item = new QuotationDetails();
