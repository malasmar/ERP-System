...
namespace CLiSales.Documents
{
    public class Proforma
    {
        ...
        public Proforma GetItem(string DB,string xLan,Guid UserKey,  Guid? Key, int DocKind, int Year)
        {
            Proforma item = new Proforma();
            item.InvoiceNo = VoucherOperation.GetMaxProforma(DB);
        ...
        public List<Proforma> GetList(string DB,int Year)
        {
            DateTime First = new DateTime(Year, 1, 1);
            DateTime Last = new DateTime(Year, 12, 31);

            List<Proforma> items = new List<Proforma>();
            string selQuery = "select top 100 percent * from SalesDocument_Proforma where [sal_InvoiceDate]>=@FirstDate and [sal_InvoiceDate]<=@LastDate order by [sal_InvoiceDate] desc,sal_InvoiceNo";
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                con.Open();
                SqlCommand com = new SqlCommand();
