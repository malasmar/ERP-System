...
namespace CLiSales.Documents
{
    public class ContractDetails
    {
        ...
        public List<ContractDetails> GetList(string DB, string xLan, Guid? Key)
        {
            List<ContractDetails> items = new List<ContractDetails>();
            if (Key == null)
                return items;

            string selQuery = "select top 100 percent * from SalesDocument_ContractDetails where [sal_OperationKey]=@key order by [sal_Index] ";
