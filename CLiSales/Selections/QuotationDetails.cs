...
namespace CLiSales.Selections
{
    public class QuotationDetails
    {
        ...
        public List<QuotationDetails> GetList(string DB,Guid? Key)
        {
            List<QuotationDetails> items = new List<QuotationDetails>();
            if (Key == null)
                return items;

            string selQuery = "select top 100 percent * from dbo.fnSalesSelection_QuotationDetails(@Key) order by [Index] ";
