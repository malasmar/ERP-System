...
namespace CLiSales.Selections
{
    public class ProformaDetails
    {
        ...
        public List<ProformaDetails> GetList(string DB,Guid? Key)
        {
            List<ProformaDetails> items = new List<ProformaDetails>();
            if (Key == null)
                return items;

            string selQuery = "select top 100 percent * from dbo.fnSalesSelection_ProformaDetails(@Key) order by [Index] ";
