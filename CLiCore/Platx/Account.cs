...
namespace CLiCore.Platx
{
    public class Account
    {
        ...
        public static Guid Insert(string DB, Guid? Parent, string Code, string Name1, string Name2)
        {
            Guid key = Guid.NewGuid();
            Account account = new Account().GetItem(DB, Parent);
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                comm.Parameters.Add("@acc_Key", SqlDbType.UniqueIdentifier).Value = iCore.IsNullRtDbNull(key);
                comm.Parameters.Add("@acc_Code", SqlDbType.NVarChar, 15).Value = Code ?? "";
                comm.Parameters.Add("@acc_Name1", SqlDbType.NVarChar, 250).Value = Name1 ?? "";
                comm.Parameters.Add("@acc_Name2", SqlDbType.NVarChar, 250).Value = Name2 ?? "";
                comm.Parameters.Add("@acc_Parent", SqlDbType.NVarChar, 15).Value = account.Code ?? "";
                comm.Parameters.Add("@acc_BalanceItem", SqlDbType.UniqueIdentifier).Value = iCore.IsNullRtDbNull(account.BalanceItem);
                ...
                comm.Parameters.Add("@acc_Financial", SqlDbType.Bit).Value = false;
                comm.Parameters.Add("@acc_Source", SqlDbType.Bit).Value = true;
                comm.Parameters.Add("@acc_Category", SqlDbType.Int).Value = account.Category;
                comm.Parameters.Add("@acc_Disable", SqlDbType.Bit).Value = false;
                comm.Parameters.Add("@acc_Status", SqlDbType.Int).Value = 0;
                comm.Parameters.Add("@acc_IsParent", SqlDbType.Bit).Value = false;
                comm.Parameters.Add("@acc_DC", SqlDbType.Bit).Value = account.DC;
                comm.Parameters.Add("@acc_CloseSubAccounts", SqlDbType.Bit).Value = account.CloseSubAccounts;
                con.Open();
        ...
        public static void Update(string DB, Guid? Key, string Name1, string Name2, bool Disable)
        {
            ...
            using (SqlConnection con = new SqlConnection(iCore.GetCon(DB)))
            {
                ...
                comm.Parameters.Add("@acc_Name1", SqlDbType.NVarChar, 250).Value = Name1 ?? "";
                comm.Parameters.Add("@acc_Name2", SqlDbType.NVarChar, 250).Value = Name2 ?? "";
                comm.Parameters.Add("@acc_Disable", SqlDbType.Bit).Value = Disable;
                con.Open();
                comm.ExecuteNonQuery();
            }
        }
