...
namespace CLiCore.Account
{
    public class User
    {
        ...
        public User GetList(string Key,Guid? Subscribe)
        {
            User item = new User();
            string selQuery = "select top 100 percent * from dbo.fnpx_Login(@Key,@Subscribe) ";
            using (SqlConnection con = new SqlConnection(iCore.Conn))
            {
                ...
                while (reader.Read())
                {
                    ...
                    item.WhatsappNumber = (string)reader["WhatsappNumber"];
                    item.Disable = (Boolean)reader["Disable"];
                    item.IsAdmin = (Boolean)reader["IsAdmin"];
        ...
        public User AppLogin(string Key, Guid? Subscribe)
        {
            User item = new User();
            string selQuery = "select top 100 percent * from dbo.fnpx_AppLogin(@Key,@Subscribe) ";
            using (SqlConnection con = new SqlConnection(iCore.Conn))
            {
                con.Open();
                SqlCommand com = new SqlCommand();
